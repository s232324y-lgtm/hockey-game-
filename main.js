...
/* harmony import */ var _components_links_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./components/links.js */ "./src/js/components/links.js");
/* harmony import */ var _components_fslightbox_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./components/fslightbox.js */ "./src/js/components/fslightbox.js");
/* harmony import */ var _components_mobile_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./components/mobile.js */ "./src/js/components/mobile.js");
...
/* harmony import */ var _components_scroll_min_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./components/scroll.min.js */ "./src/js/components/scroll.min.js");
/* harmony import */ var _components_select_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./components/select.js */ "./src/js/components/select.js");
/* harmony import */ var _components_sum_js__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./components/sum.js */ "./src/js/components/sum.js");
/* harmony import */ var _components_servers_online_js__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./components/servers_online.js */ "./src/js/components/servers_online.js");
/* harmony import */ var _components_yandex_metrika_js__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./components/yandex_metrika.js */ "./src/js/components/yandex_metrika.js");
  ...
  try {
    ...
    linkElements.forEach(el => {
      ...
      if (links[key]) {
        ...
        console.warn(`Ссылка с ключом "${key}" отсутствует в JSON.`);
      }
    });
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", function () {
  // Получаем scroll-контейнер: document.scrollingElement или резервные варианты
  const scrollElement = (() => {
    if ("scrollingElement" in document) return document.scrollingElement;
    const html = document.documentElement;
    const prevScrollTop = html.scrollTop;
    html.scrollTop += 1;
    const isChanged = html.scrollTop > prevScrollTop;
    html.scrollTop = prevScrollTop;
    return isChanged ? html : document.body;
  })();

  // Вычисляет расстояние до элемента по якорю
  function getScrollData(anchor) {
    const start = scrollElement.scrollTop;
    if (anchor.length < 2) return new Map([["start", start], ["delta", -start]]); // href="#"

    const target = document.querySelector(anchor);
    if (!target) return;
    const rectTop = target.getBoundingClientRect().top;
    const maxScroll = scrollElement.scrollHeight - window.innerHeight;
    const delta = Math.min(rectTop, maxScroll - start);
  ...
  function smoothScroll(link) {
    ...
    // ⛔ Пропускаем внешние или некорректные ссылки
    if (!href || !href.startsWith("#")) return;
    const scrollData = getScrollData(href);
    ...
    function scrollFrame(currentTime) {
      ...
      const ease = -Math.pow(2, -10 * progress) + 1; // easeOutExpo

      const scrollStart = scrollData.get("start");
      const scrollDelta = scrollData.get("delta");
      scrollElement.scrollTop = Math.round(scrollStart + scrollDelta * ease);
      if (elapsed < duration) {
...
/***/ "./src/js/components/servers_online.js":
/*!*********************************************!*\
  !*** ./src/js/components/servers_online.js ***!
  \*********************************************/
/***/ ((__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) => {
...
__webpack_require__.r(__webpack_exports__);
document.addEventListener("DOMContentLoaded", function () {
  fetch("./servers_stats/online.json").then(response => {
    if (!response.ok) throw new Error("Network response was not ok");
    return response.json();
  }).then(data => {
    const server1 = data.server_1?.online || 0;
    const server2 = data.server_2?.online || 0;
    const server3 = data.server_3?.online || 0;
    const totalOnline = server1 + server2 + server3;
    console.log("server1: ", server1);
    console.log("server2: ", server2);
    console.log("server3: ", server3);
    console.log("Общий онлайн: ", totalOnline);

    // Обновляем элементы на странице
    const headerOnline = document.querySelector(".header__online_count");
    const footerOnline = document.querySelector(".footer__online_count");
    if (headerOnline) headerOnline.textContent = totalOnline;
    if (footerOnline) footerOnline.textContent = totalOnline;
  }).catch(error => {
    console.error("Ошибка загрузки онлайн-данных:", error);
  });
});

/***/ }),

/***/ "./src/js/components/sum.js":
/*!**********************************!*\
  !*** ./src/js/components/sum.js ***!
  \**********************************/
/***/ ((__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const doubleBox = document.querySelector(".form-payment__double");
  const doubleText = document.querySelector(".form-payment__double_text");
  const totalText = document.querySelector(".form-payment__total");
  inputSum.addEventListener("input", () => {
    const sumValue = parseFloat(inputSum.value) || 0;
    if (doubleBox && getComputedStyle(doubleBox).display !== "none") {
      const doubleAmount = sumValue * 2;
      doubleText.textContent = `${doubleAmount} ₽`;
    } else if (doubleText) {
      doubleText.textContent = `0 ₽`;
    }
    const totalAmount = sumValue;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  playButton.addEventListener("click", event => {
    ...
    videoPreview.style.display = "none"; // Скрываем превью
  ...
  videoElement.addEventListener("ended", () => {
    ...
    videoPreview.style.display = "flex"; // Показываем превью
    playButton.style.display = "flex"; // Показываем кнопку Play
  });
});

/***/ }),

/***/ "./src/js/components/yandex_metrika.js":
/*!*********************************************!*\
  !*** ./src/js/components/yandex_metrika.js ***!
  \*********************************************/
/***/ ((__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) => {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const downloadLink = document.querySelector('a[data-link="download_button"]');
  ...
  if (downloadLink) {
    downloadLink.addEventListener('click', function(e) {
      ...
      const downloadUrl = downloadLink.getAttribute('href'); // ссылка на APK

      // если нет ссылки — ничего не делаем
      if (!downloadUrl) return;

      // если нет реферального параметра — просто качаем без учёта
      if (!ref) {
      ...
      fetch('/referral_download.php', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ ref_name: ref })
      }).finally(() => {
        // После отправки (успешно или нет) — запускаем скачивание
        window.location.href = downloadUrl;
      });
    });
  }
  ...
  document.getElementById('download_button').addEventListener('click', function () {
    ...
    ym(102905395, 'reachGoal', 'download_game');
    const ref = new URLSearchParams(window.location.search).get('ref');
    if (!ref) return;
	
	console.log("fetch referral_download");
    fetch('/referral_download.php', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
